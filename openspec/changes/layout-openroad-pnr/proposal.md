...
- The executor:
  ...
  3. Runs OpenROAD place-and-route inside Docker using the `cgra/cgra-flow:web-ui` image, with mflowgen orchestrating the flow through to the `final_all` step.
...
- **Docker image**: must be `cgra/cgra-flow:web-ui` (already built); `runner/.env` `DOCKER_IMAGE` is already set.
