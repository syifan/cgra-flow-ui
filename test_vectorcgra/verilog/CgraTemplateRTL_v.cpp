...
#if VLINETRACE
#include "obj_dir_CgraTemplateRTL/VCgraTemplateRTL__Syms.h"
#include "svdpi.h"
#endif
...
extern "C" {
  ...
  #if VLINETRACE
  void trace( VCgraTemplateRTL_t *, char * );
  #endif
...
VCgraTemplateRTL_t * create_model( const char *vcd_filename ) {

  VCgraTemplateRTL_t * m;
  VCgraTemplateRTL   * model;

  Verilated::randReset( 0 );
  Verilated::randSeed( 0 );

  m     = (VCgraTemplateRTL_t *) malloc( sizeof(VCgraTemplateRTL_t) );
  model = new VCgraTemplateRTL();
  ...
  m->address_lower = &model->address_lower;
  m->address_upper = &model->address_upper;
  m->cgra_id = &model->cgra_id;
  m->clk = &model->clk;
  m->reset = &model->reset;
  ...
  m->recv_from_inter_cgra_noc___05Fmsg = model->recv_from_inter_cgra_noc___05Fmsg;
  m->recv_from_inter_cgra_noc___05Frdy = &model->recv_from_inter_cgra_noc___05Frdy;
  m->recv_from_inter_cgra_noc___05Fval = &model->recv_from_inter_cgra_noc___05Fval;
  ...
  m->send_to_inter_cgra_noc___05Fmsg = model->send_to_inter_cgra_noc___05Fmsg;
  m->send_to_inter_cgra_noc___05Frdy = &model->send_to_inter_cgra_noc___05Frdy;
  m->send_to_inter_cgra_noc___05Fval = &model->send_to_inter_cgra_noc___05Fval;
...
void destroy_model( VCgraTemplateRTL_t * m ) {

  #if VM_COVERAGE
    VerilatedCov::write( "coverage.dat" );
  #endif

  VCgraTemplateRTL * model = (VCgraTemplateRTL *) m->model;
...
void comb_eval( VCgraTemplateRTL_t * m ) {
...
void seq_eval( VCgraTemplateRTL_t * m ) {
...
#if VLINETRACE
void trace( VCgraTemplateRTL_t * m, char* str ) {
  ...
  svSetScope( &model->__VlSymsp->__Vscope_CgraTemplateRTL__v );
