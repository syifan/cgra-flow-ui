...
test.describe('Mapping job lifecycle', () => {
  test('completes full mapping job cycle: queued -> running -> success', async ({ workspacePage: page }) => {
    ...
    try {
      ...
      await expect(page.getByText('Select benchmarks to map on your CGRA design:')).toBeVisible();

      // Verify initial state - no pending jobs
      await expect(page.locator('.MuiChip-label:has-text("No pending jobs")')).toBeVisible();
      ...
      await expect(page.locator('text=Mapping job queued successfully')).toBeVisible({ timeout: 5000 });

      // ===== PHASE 1: Verify QUEUED state (before runner starts) =====

      // Verify job is queued - check for locked indicator with "queued" text
      await expect(page.locator('.MuiChip-label:has-text("1 job queued")')).toBeVisible({ timeout: 5000 });
      ...
      await expect(page.getByRole('button', { name: 'Start Mapping' })).toBeDisabled();
      ...
      await expect(page.locator('.MuiChip-label:has-text("1 job running")')).toBeVisible({ timeout: 15000 });
      ...
      await expect(page.locator('.MuiChip-label:has-text("No pending jobs")')).toBeVisible({ timeout: 60000 });

      // Verify the Mapping tab shows "Success" status
      await expect(page.locator('.MuiChip-label:has-text("Success")')).toBeVisible({ timeout: 5000 });

      // Verify UI is unlocked - Start Mapping button should be enabled again
      await expect(page.getByRole('button', { name: 'Start Mapping' })).toBeEnabled();
